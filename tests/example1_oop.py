...
def replace_missing_data(df: Union[np.ndarray, pd.DataFrame], column_list: List[str], replace_value: Dict[str, str])->pd.DataFrame:
    """
...
class Classifier():
    ...
    def score(self, predicted_label: nd.array, actual_label: nd.array, list_metrics: List[str])-> Dict[str, Union[float, np.ndarray]]:
        metrics_dict = dict
